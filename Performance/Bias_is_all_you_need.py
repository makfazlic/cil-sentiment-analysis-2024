...
def Datapoint_Biased(ac_Datapoint,Biasing_Step_Data):
    with torch.no_grad():
        ...
        for Biasing_Step_DataPoint in Biasing_Step_Data:
            ...
            Embeddings=Biasing_Step_DataPoint[0](Tokenized)["sentence_embedding"][0] 
...
def Update_Biasing_Step_Data(Biasing_Step_Data,embedding_model,adapter_Model,clustering_Model,Biasing_Determining_Time,Biasing_Data,file_names):
    ...
    with torch.no_grad():
        ...
        Start_Time_Biasing=time.clock_gettime(clk_id)
        all_datapoints_biased=0
        while time.clock_gettime(clk_id)-Start_Time_Biasing<Biasing_Determining_Time:
            ...
            try:
                with torch.no_grad():
                    ...
                    Embeddings=embedding_model(Tokenized)["sentence_embedding"]
    ...
    Biasing_Step_Data[-1].append(copy.deepcopy(embedding_model))
...
def Train_New_Model(Training_Data,Genom,Number_Training_Batches,Allowed_Time_Cluster_Init,Test_Data,Biasing_Step_Data,loss_statistic,Biasing_Determining_Time,Batchsize=8):
    ...
    if len(Biasing_Step_Data)==0 or acGenom["Load_Finetuned_Model"]==0:
        embedding_model = SentenceTransformer("./local_models/"+Genom["Embedding_Model"],device=Device_of_Model)
    else:
        #print("load")
        embedding_model = SentenceTransformer(Biasing_Step_Data[-1][3][0],device=Device_of_Model)

    #Prepare LoRA if needed
    if Genom["Finetuning_Method"]=="LoRA":
        lora_config = LoraConfig(
            r=Genom["Lora_R"],
            lora_alpha=round(Genom["Lora_R"]*Genom["Lora_Alpha_to_R"]),
            target_modules=["q", "v"],
            lora_dropout=0.1,
            bias="none",
            modules_to_save=["classifier"],  # which weights not to freeze
            task_type=TaskType.SEQ_CLS,  # probably not really needed
        )

        embedding_model= get_peft_model(embedding_model, lora_config)
    ...
    if Genom["Clustering"]==1:
        if Genom["K-Means_Initialization"]==0:
            ...
            Pos_Embeddings=[]
            Neg_Embeddings=[]
            Start_Time_Clustering_Init=time.clock_gettime(clk_id)
            #print("HEy")
            while Allowed_Time_Cluster_Init>time.clock_gettime(clk_id)-Start_Time_Clustering_Init:
                #print(time.clock_gettime(clk_id)-Start_Time_Clustering_Init,end="\r",flush=True)
                ...
                with torch.no_grad():
                    ...
                    Embeddings=embedding_model(Tokenized)["sentence_embedding"][0].tolist()
    ...
    while True:
        ...
        while max(Actual_Position_Train)<=Max_Position:
            ...
            Classification=adapter_Model(Embeddings["sentence_embedding"])
            ...
            if Genom["Clustering"]==1: #Bias Loss
                Cluster_Model_Output=clustering_Model(Embeddings["sentence_embedding"])
                loss_Biasing=Loss_Clustering_Total(Cluster_Model_Output,Genom["Clustering_Loss_to_Center_Loss"])
                loss_ED=loss_embedding_similarity(Embeddings["sentence_embedding"],labels)
...
def Eval_New_Model(Test_Data,embedding_model,adapter_Model,Allowed_Time,Biasing_Step_Data,Batchsize=8):
    with torch.no_grad():
        Start_Time_Testing=time.clock_gettime(clk_id)
        ...
        while max(Actual_Position_Test)<=Max_Position:
            ...
            if time.clock_gettime(clk_id)-Start_Time_Testing>=Allowed_Time:
                break
